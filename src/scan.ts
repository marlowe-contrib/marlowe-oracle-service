...
export async function getActiveContracts(
    ...
    const timeBefore1Minute: Date = new Date(
        currentTime.getTime() - 1 * 60 * 1000
    );

    const timeAfter1Minute: Date = new Date(
        currentTime.getTime() + 1 * 60 * 1000
    );
    ...
    for (const contract of allContractHeaders) {
        const nextSteps = await client.getNextStepsForContract(
            contract.contractId,
            mkEnvironment(timeBefore1Minute)(timeAfter1Minute),
            []
        )();
                ...
                value.applicable_inputs.choices.forEach((choice) => {
                    if (
                        ...
                        const newRequest: OracleRequest = {
                            ...
                            invalidBefore: timeBefore1Minute,
                            invalidHereafter: timeAfter1Minute,
                            bridgeUtxo: none,
                        };
